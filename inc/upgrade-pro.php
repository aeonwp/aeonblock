...
class AeonBlock_Theme_Notice {
	 ...
	 *
	 * AeonBlock_Theme_Notice constructor.
	 */
	...
	public static function get_theme_lists() {
		...
		$theme_lists = array(
			'aeonblock'      => 'https://aeonwp.com/aeonblock-plus/',
		);
	...
	public function pro_theme_notice() {
		...
		$option = get_option( 'aeonblock_theme_notice_start_time' );

		if ( ! $option ) {
			update_option( 'aeonblock_theme_notice_start_time', time() );
		}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'aeonblock-notice', get_template_directory_uri() . '/css/upgrade-pro.css', array(), '4.5.0' );
	}
	...
	public function pro_theme_notice_markup() {
		...
		$theme_notice_start_time = get_option( 'aeonblock_theme_notice_start_time' );
		$ignore_notice_permanent = get_user_meta( $this->current_user_data->ID, 'aeonblock_upgrade_pro_theme_notice_ignore', true );
		$ignore_notice_partially = get_user_meta( $this->current_user_data->ID, 'aeonblock_upgrade_pro_theme_notice_partial_ignore', true );
		...
		<div class="notice updated pro-theme-notice">
			<p>
				<?php
				$pro_link = '<a target="_blank" href=" ' . esc_url( $theme_lists[ $current_theme ] ) . ' ">' . esc_html__( 'upgrade to pro', 'aeonblock' ) . '</a>';
			...
			<div class="links">
				<a href="<?php echo esc_url( $theme_lists[ $current_theme ] ); ?>" class="btn button-primary"
				   target="_blank">
					...
					<span><?php esc_html_e( 'Upgrade To Pro', 'aeonblock' ); ?></span>
				</a>

				<a href="?aeonblock_upgrade_pro_theme_notice_partial_ignore=1" class="btn button-secondary">
					<span class="dashicons dashicons-calendar"></span>
					<span><?php esc_html_e( 'Maybe later', 'aeonblock' ); ?></span>
				</a>
			</div>

			<a class="notice-dismiss" href="?aeonblock_upgrade_pro_theme_notice_ignore=1"></a>
		</div>
	...
	public function pro_theme_notice_partial_ignore() {
		...
		if ( isset( $_GET['aeonblock_upgrade_pro_theme_notice_partial_ignore'] ) && '1' == $_GET['aeonblock_upgrade_pro_theme_notice_partial_ignore'] ) {
			update_user_meta( $user_id, 'aeonblock_upgrade_pro_theme_notice_partial_ignore', time() );
		}
	}
	...
	public function pro_theme_notice_ignore() {
		...
		if ( isset( $_GET['aeonblock_upgrade_pro_theme_notice_ignore'] ) && '1' == $_GET['aeonblock_upgrade_pro_theme_notice_ignore'] ) {
			update_user_meta( $user_id, 'aeonblock_upgrade_pro_theme_notice_ignore', time() );
		}
	}
}
new AeonBlock_Theme_Notice();
